...
```shell
docker container exec --name 容器名 -p 宿主机端口号:容器端口号 -v 宿主机目录:容器目录 -dit 镜像ID [命令]
```
...
* 容器名
    ...
    > 注：
    >
    > 此处的`容器名`是`docker-compose`文件定义的服务(容器)名，只适用于该`docker-compose`文件，与`docker container ls -a`看到的`容器名`不一定一致。
