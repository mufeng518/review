...
```shell
docker build [-f Dockerfie文件路径] -t 仓库名:标签名 .
```
...
* `容器名`
    ...
    > 注：
    >
    > 此处的`容器名`是`docker-compose`定义的容器名，只适用于该`docker-compose`文件，与`docker container ls -a`看到的容器名不一定一致。
...
* `build`
    ...
    * 单个镜像文件
        ...
        services:
          容器名:
            build: Dockerfile文件的相对目录路径或绝对目录路径
        ```
    ...
    * 多个镜像文件
        ...
        services:
          容器名:
            build:
              context: Dockerfile文件的相对目录路径或绝对目录路径
