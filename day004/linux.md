...
* 概述
    ...
    ssh是一个远程登录协议，默认的配置文件存放在`/etc/ssh/ssh_confg`。
...
```shell
yum/sudo pip [-y] install openssh-server
```

##### 服务相关命令

```shell
systemctl unit start/stop/restart/status
...
```shell
systemctl iptables start/stop
service start/stop iptables
```
...
> 附：命令结果解析
>
> 当前系统登录时长、当前登录系统的用户数量和当前系统平均负载情况
>
> 任务情况
>
> CPU情况
