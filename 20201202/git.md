...
```shell
git checkout -- 工作目录或文件路径 ...
```

> 注：
>
> 当版本库里面的目录或文件被修改或删除后未被提交到暂存区时，那么撤销后将会回退到最后一次`git commit`时的状态，当版本库里面的目录或文件被提交到暂存区后又被修改或删除时，那么撤销后将会回退到最后一次`git add`时的状态。
