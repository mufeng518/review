...
```shell
docker loging -u 用户名 -p 密码 [镜像仓库地址]
```

> 注：
>
> 当`镜像仓库地址`省略时，那么默认为`镜像仓库地址`为`DokerHub`。
...
```shell
docker image tag 用户名:仓库名 [镜像仓库地址/][用户名/]仓库名:标签名
```
...
```shell
docker container run -dit -p 宿主机端口号:容器端口号 --name 容器名 -v 宿主机目录:容器目录 镜像ID [命令]
```
...
```shell
docker import ./文件路径.tar 仓库名:标签名
```
