...
* 上传
    ...
    ```shell
    ssh 源路径 用户名@ip地址:目标路径
    ```

* 下载

    ```shell
    ssh 用户名@ip地址:源路径 目标路径
    ```
...
> 附：
>
> 当前系统已运行时长、当前登录系统的用户数量和当前系统的平均负载情况；
>
> CPU情况
>
> 任务情况
...
* 概述
    ...
    `sourcce`命令也被称为点命令(即`.`)，通常用于重新执行被修改的初始化文件。
