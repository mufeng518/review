...
```shell
docker container --name 容器名 -v 宿主机目录:容器目录 -p 宿主机端口号:容器端口号 -dit 镜像ID [命令]
```

##### 查看

```shell
docker coantainer ls -a
```
...
```shell
docker build [-f Dockerfile问价路径] -t 仓库名:标签名 .
```