...
* 编译性语言编写的程序在执行之前需要一个专门的编译过程，先将程序代码翻译成一个机器语言的文件，当程序执行时，那么不会再对程序重新编译，只需要执行编译的结果就行了。
...
内存也被称为内存储器，是计算机中最重要的部件之一，计算机中所有程序的执行都是在内存中进行的，并且它是计算机与CPU沟通的桥梁。
...
* 概念
    ...
    变量是程序运行期间可操作的存储空间的名称，并且是程序运行期间不可以改变的数据。
...
* 转换成字符串
    ...
    ```python
    str(数值/布尔/None/列表/元组/字典/集合变量)
    ```

##### 常用数学函数

```python
max/min(数值1, .../字符串/(字符串或数值)列表/元组/字典/集合变量)
sum(数值列表/元组/字典/集合变量)
abs(数值)
pow(数值, n次方)
round(数值, 保留小数位数)
...
* 三元表达式
    ...
    ```python
    (变量名 = )条件表达式时成立返回的值 if 条件语句 else 条件表达式不成立时返回的值
    ```
