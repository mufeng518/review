...
```shell script
docker container run --name 容器名 -p [宿主机ip地址:]端口号:容器端口号 -v 宿主机目录:容器目录 -dit --rm 镜像ID [命令]
```
...
* 所有
    ...
    ```shell script
    docker container start/stop/restart/kill $(docker container ps -aq)
    ```
...
* 所有
    ...
    ```shell script
    docker contaienr rm -f $(docker container ps -aq)
    ```
...
```shell script
docker commit --author "作者" --message "信息" -dit 容器ID 仓库名:标签名
```
...
```shell script
docker build [-f Dockerfile文件] -t 仓库名:标签名 .
```
