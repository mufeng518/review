...
* 概述
    ...
    软链接是既可以链接目录，也可以链接普通文件，但源文件删除会使软链接变成死链接。
...
* 概述
    ...
    `ssh`是一个远程连接协议，配置文件默认存放在`/etc/ssh/ssh_config`。
...
* 将上传的公钥重定向到对方主机的`~/.ssh/authorized_keys`文件中
...
```shell
sudo apt/yum install openssh-server
```
...
> 附：
>
> 当前系统已运行时长、登录当前系统的用户数量和当前系统平均负载情况
>
> CPU情况
>
> 任务情况
...
* 概述
    ...
    `source`命令通常用于重新激活被修改的配置文件，使其立即生效。
