...
版本库也被称为仓库，也可以理解成一个目录，版本库里面的所有目录或文件都可以被Git来管理，即版本库里面每个目录或文件的修改或删除，Git都可以来跟踪。
...
> 注：
>
> 当版本库里面又被修改或删除的目录或文件时，那么返回被修改或删除的目录或文件。
...
> 注:
>
> 当文件被修改或删除后未被添加到暂存区时，那么撤销后将会回退到最后一次`git commit`时的状态，当目录或文件被添加到暂存区后又被修改或删除时，那么撤销后将会回退到最后一次`git add`时的状态。
