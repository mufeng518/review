...
Docker是一个开源的容器引擎，通过Docker可以快速地构建一个轻量的、可移植的、自给自足的容器。
...
```shell
docker image pull origin [镜像仓库地址/][用户名/]仓库名[:标签名]
```

##### 推送

```shell
docker image push origin [镜像仓库地址/][用户名/]仓库名:标签名
```
...
* 所有
    ...
    ```shell
    docker container rm -f $(docker contianer ps -aq)
    ```
