...
* 单个
    ...
    ```shell
    docker start/stop/kill/restart 容器ID
    ```

* 所有

    ```shell
    docker start/stop/kill/restart $(docker container ps -aq)
    ```
...
```shell
docker commmit --author "作者" --message "信息" 容器ID 仓库名:标签名
```
...
* 容器名
    ...
    > 注：
    >
    > 此处的容器名是`docker-compose`文件定义的容器名难过，只适用于该`docker-compose`文件，与`docker container ls -a`看到的容器名不一定一致。
