...
> 注：
>
> 当版本库里面的文件被修改后未添加到暂存区时，那么撤销后将会回退到最后一次`git commit`时的状态，当版本库里面的文件被添加到暂存后又被修改时，那么撤销后将会回退到最后一次`git add`时的状态。
...
引入了分支以后，可以更好地解决各分支功能模块间开发进度的影响。
